...
const DryFruits = () => {
  ...
  const dryfruits = useMemo(() => allProducts.filter(p => p.category.includes('Dry Fruits') || ['Aam Dry Fruits', 'Seeds Walay', 'Meethy Dry Fruits', 'Khaas Dry Fruits'].includes(p.category)), [allProducts]);
  ...
  const filters = [
    ...
    { id: 'Aam Dry Fruits', name: 'Aam Dry Fruits', nameEnglish: 'Common Dry Fruits' },
    { id: 'Seeds Walay', name: 'Seeds Walay', nameEnglish: 'Seeds' },
    { id: 'Meethy Dry Fruits', name: 'Meethy Dry Fruits', nameEnglish: 'Sweet Dry Fruits' },
    { id: 'Khaas Dry Fruits', name: 'Khaas Dry Fruits', nameEnglish: 'Special Dry Fruits' }
  ];
  const weightOptions = ['250g', '500g', '1kg'];

  const filteredProducts = useMemo(() => {
    if (activeFilter === 'Sab') return dryfruits;
    return dryfruits.filter(f => f.category === activeFilter);
  }, [activeFilter, dryfruits]);
  ...
  const handleWhatsAppSingle = (product: Product) => {
    ...
    const message = `Assalam o Alaikum! Main AgriConnect say yeh order karna chahta hoon: ${product.name} [250g] [Price: Rs. ${finalPrice * 0.25}]`;
    window.open(`https://wa.me/923019515764?text=${encodeURIComponent(message)}`, '_blank');
  };

  return (
    <div className="pt-[72px]">
      <DiscountBanner products={dryfruits} />
      {/* Page Header */}
      <section className="relative h-64 md:h-80 flex items-center justify-center overflow-hidden ac-fade-in">
        <div 
          className="absolute inset-0 bg-cover bg-center bg-no-repeat"
          style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1596591606975-97ee5cef3a1e?auto=format&fit=crop&q=80&w=1920)' }}
        >
        ...
        <div className="relative z-10 text-center text-white px-4">
          ...
          >
            {language === 'romanUrdu' ? 'Mazaydar Dry Fruits' : 'Delicious Dry Fruits'}
          </motion.h1>
          <motion.p 
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
          ...
          >
            {language === 'romanUrdu' ? '100% Asli Aur Taaza Dry Fruits' : '100% Authentic & Fresh Dry Fruits'}
          </motion.p>
        </div>
      </section>
              ...
              {filteredProducts.map((product, idx) => {
                ...
                return (
                  ...
                  <div className="p-4 md:p-6 flex flex-col flex-1">
                    ...
                    <p className="text-xs text-gray-500 mb-3">{t('wazan')}: 250g / 500g / 1kg</p>
                    ...
                    <div className="mt-auto">
                      ...
                      <div className="space-y-2">
                        <button 
                          onClick={() => handleAddToCart(product, 1, '250g')}
      ...
      <section className="py-16 bg-white ac-slide-up">
        <div className="max-w-7xl mx-auto px-4">
          <div className="bg-red-600 rounded-[2.5rem] p-8 md:p-16 text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
              <div className="text-center md:text-left">
                ...
                <h2 className="text-4xl md:text-6xl font-serif font-bold mb-4">
                  {language === 'romanUrdu' ? 'Chilgoza Par 10% Discount!' : '10% Discount on Pine Nuts!'}
                </h2>
                <p className="text-xl text-white/90 max-w-xl">
                  {language === 'romanUrdu' 
                    ? 'Aala quality ke chilgozay ab aur bhi saste. Limited time offer!' 
                    : 'Premium quality pine nuts now even cheaper. Limited time offer!'}
                </p>
                <div className="mt-8 flex items-center gap-4 justify-center md:justify-start">
                  <span className="bg-white text-red-600 px-6 py-2 rounded-full font-black text-2xl">Rs. 4500 / kg</span>
                  <span className="text-white/60 line-through text-xl font-bold">Rs. 5000</span>
                </div>
              </div>
              <button className="bg-white text-red-600 px-10 py-5 rounded-2xl font-bold text-xl flex items-center gap-3 hover:bg-gray-100 transition-all shadow-xl">
      ...
      <ShopDiscountBanner products={dryfruits} category="Dry Fruits" />
