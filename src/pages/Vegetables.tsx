...
const Vegetables = () => {
  ...
  const vegetables = useMemo(() => allProducts.filter(p => p.category.includes('Sabziyaan') || ['Rozmarra Wali', 'Patti Wali', 'Aam Sabziyaan', 'Khaas Sabziyaan'].includes(p.category)), [allProducts]);
  ...
  const filters = [
    ...
    { id: 'Rozmarra Wali', name: 'Rozmarra Wali', nameEnglish: 'Daily Use' },
    { id: 'Patti Wali', name: 'Patti Wali', nameEnglish: 'Leafy Vegetables' },
    { id: 'Aam Sabziyaan', name: 'Aam Sabziyaan', nameEnglish: 'Common Vegetables' },
    { id: 'Khaas Sabziyaan', name: 'Khaas Sabziyaan', nameEnglish: 'Special Vegetables' }
  ];

  const filteredProducts = useMemo(() => {
    if (activeFilter === 'Sab') return vegetables;
    return vegetables.filter(v => v.category === activeFilter);
  ...
  return (
    <div className="pt-[72px]">
      <DiscountBanner products={vegetables} />
      {/* Page Header */}
      <section className="relative h-64 md:h-80 flex items-center justify-center overflow-hidden ac-fade-in">
        ...
        <div className="relative z-10 text-center text-white px-4">
          ...
          >
            {language === 'romanUrdu' ? 'Taaza Sabziyaan' : 'Fresh Vegetables'}
          </motion.h1>
          <motion.p 
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
          ...
          >
            {language === 'romanUrdu' ? 'Seedha Punjab Kay Khetton Say' : 'Directly From Punjab Fields'}
          </motion.p>
        </div>
      </section>
            ...
            <AnimatePresence mode="popLayout">
              {filteredProducts.map((product, idx) => {
                ...
                return (
                  ...
                  {/* Image Container */}
                  <div 
                    className="relative h-44 md:h-60 overflow-hidden cursor-pointer group/img"
                    onClick={() => setSelectedProduct(product)}
                  >
                    <img 
                      src={product.image} 
                      alt={language === 'romanUrdu' ? product.name : product.nameEnglish} 
                      className="w-full h-full object-cover group-hover:scale-105 group-hover/img:scale-110 transition-transform duration-1000 ease-in-out"
                    ...
                    <div className="absolute top-3 right-3 bg-white/95 backdrop-blur-md px-2.5 py-1.5 rounded-xl flex items-center gap-1.5 shadow-lg border border-white/20">
                      <Star className="w-3.5 h-3.5 text-agri-orange fill-agri-orange" />
                      <span className="text-xs font-black text-gray-900">{product.rating}.0</span>
                    </div>
                  </div>
      ...
      <section className="py-16 bg-white ac-slide-up">
        <div className="max-w-7xl mx-auto px-4">
          <div className="bg-red-500 rounded-[2.5rem] p-8 md:p-16 text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
              <div className="text-center md:text-left">
                ...
                <h2 className="text-4xl md:text-6xl font-serif font-bold mb-4">
                  {language === 'romanUrdu' ? 'Aalu Par 30% Discount!' : '30% Discount on Potatoes!'}
                </h2>
                <p className="text-xl text-white/90 max-w-xl">
                  {language === 'romanUrdu' 
                    ? 'Punjab ke khetton se taaza aalu ab aur bhi saste. Sirf aaj ke liye!' 
                    : 'Fresh potatoes from Punjab fields now even cheaper. For today only!'}
                </p>
                <div className="mt-8 flex items-center gap-4 justify-center md:justify-start">
                  <span className="bg-white text-red-500 px-6 py-2 rounded-full font-black text-2xl">Rs. 56 / kg</span>
                  <span className="text-white/60 line-through text-xl font-bold">Rs. 80</span>
                </div>
              </div>
              <button className="bg-white text-red-500 px-10 py-5 rounded-2xl font-bold text-xl flex items-center gap-3 hover:bg-agri-cream transition-all shadow-xl">
      ...
      <ShopDiscountBanner products={vegetables} category="Sabziyaan" />
