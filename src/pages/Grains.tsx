...
const Grains = () => {
  ...
  const grains = useMemo(() => allProducts.filter(p => p.category.includes('Anaaj') || ['Aam Anaaj', 'Daliyan', 'Chawal Ki Kismein', 'Khaas Anaaj'].includes(p.category)), [allProducts]);
  ...
  const filters = [
    ...
    { id: 'Aam Anaaj', name: 'Aam Anaaj', nameEnglish: 'Common Grains' },
    { id: 'Daliyan', name: 'Daliyan', nameEnglish: 'Pulses' },
    { id: 'Chawal Ki Kismein', name: 'Chawal Ki Kismein', nameEnglish: 'Rice Varieties' },
    { id: 'Khaas Anaaj', name: 'Khaas Anaaj', nameEnglish: 'Special Grains' }
  ];
  const weightOptions = ['1kg', '5kg', '10kg'];

  const filteredProducts = useMemo(() => {
    if (activeFilter === 'Sab') return grains;
    return grains.filter(f => f.category === activeFilter);
  }, [activeFilter, grains]);
  ...
  return (
    <div className="pt-[72px]">
      <DiscountBanner products={grains} />
      {/* Page Header */}
      <section className="relative h-64 md:h-80 flex items-center justify-center overflow-hidden ac-fade-in">
        <div 
          className="absolute inset-0 bg-cover bg-center bg-no-repeat"
          style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?auto=format&fit=crop&q=80&w=1920)' }}
        >
        ...
        <div className="relative z-10 text-center text-white px-4">
          ...
          >
            {language === 'romanUrdu' ? 'Taaza Anaaj' : 'Fresh Grains'}
          </motion.h1>
          <motion.p 
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
              ...
              {filteredProducts.map((product, idx) => {
                ...
                return (
                  ...
                  <div className="p-4 md:p-6 flex flex-col flex-1">
                    ...
                    <p className="text-xs text-gray-500 mb-3">{t('wazan')}: 1kg / 5kg / 10kg</p>
      ...
      <section className="py-16 bg-white ac-slide-up">
        <div className="max-w-7xl mx-auto px-4">
          <div className="bg-agri-green rounded-[2.5rem] p-8 md:p-16 text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
              <div className="text-center md:text-left">
                ...
                <h2 className="text-4xl md:text-6xl font-serif font-bold mb-4">
                  {language === 'romanUrdu' ? 'Basmati Chawal Par 15% Discount!' : '15% Discount on Basmati Rice!'}
                </h2>
                <p className="text-xl text-white/90 max-w-xl">
                  {language === 'romanUrdu' 
                    ? 'Punjab ke aala basmati chawal ab aur bhi saste. Ghar baithe mangwayein!' 
                    : 'Punjab\'s premium Basmati rice now even cheaper. Order from home!'}
                </p>
                <div className="mt-8 flex items-center gap-4 justify-center md:justify-start">
                  <span className="bg-white text-agri-green px-6 py-2 rounded-full font-black text-2xl">Rs. 295 / kg</span>
                  <span className="text-white/60 line-through text-xl font-bold">Rs. 350</span>
                </div>
              </div>
              <button className="bg-white text-agri-green px-10 py-5 rounded-2xl font-bold text-xl flex items-center gap-3 hover:bg-agri-cream transition-all shadow-xl">
      ...
      <ShopDiscountBanner products={grains} category="Anaaj" />
